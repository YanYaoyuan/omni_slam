...
void Preprocess::livox_pointcloud2_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  auto stamp_to_sec = [this](double stamp) {
  ...
  for (const auto &point : pl_orig.points)
  {
    if (std::isfinite(point.timestamp))
      scan_begin = std::min(scan_begin, stamp_to_sec(point.timestamp));
  }
  ...
  for (uint i = 0; i < pl_orig.points.size(); ++i)
  {
    ...
    double offset_ms = (stamp_to_sec(src.timestamp) - scan_begin) * 1000.0;
    added_pt.curvature = static_cast<float>(std::max(0.0, offset_ms));
